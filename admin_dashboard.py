...
def api_documents(kind: str = Query(default="all", pattern="^(all|invoice|quote)$"), search: str = Query(default=""), _: str = Depends(admin_login)) -> JSONResponse:
...
:root{color-scheme:dark;--bg:#07111f;--panel:#102238;--muted:#9fb1c5;--green:#43df87;--blue:#43a5ff;--border:#23364e}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:#fff;font-family:Arial,sans-serif}.page{max-width:1800px;margin:auto;padding:20px}.head{display:flex;justify-content:space-between;gap:15px;align-items:center}.headright{display:flex;align-items:center;gap:10px;flex-wrap:wrap}h1{margin:0}.live{color:var(--green);font-weight:700}.navbtn{display:inline-block;text-decoration:none;background:#152b46;color:#fff;border:1px solid var(--border);border-radius:9px;padding:10px 14px;font-weight:700}.navbtn:hover{background:#1d3a5f}.navbtn.primary{background:var(--blue);color:#00152b}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px;margin-top:18px}.card,.panel{background:var(--panel);border:1px solid var(--border);border-radius:15px}.card{padding:18px}.label,.hint{color:var(--muted);font-size:13px}.value{font-size:31px;font-weight:800;margin-top:7px}.tabs{display:flex;gap:8px;flex-wrap:wrap;margin:18px 0}button,input,select{background:#152b46;color:#fff;border:1px solid var(--border);border-radius:9px;padding:9px 11px}.active{background:var(--blue);color:#00152b;font-weight:700}.panel{padding:18px;margin-top:14px}.layout{display:grid;grid-template-columns:1.2fr .8fr;gap:14px}.chart{height:220px;display:flex;align-items:flex-end;gap:6px;overflow:auto}.barwrap{min-width:28px;flex:1;text-align:center}.bar{background:linear-gradient(180deg,var(--blue),#2671ff);border-radius:7px 7px 2px 2px;min-height:3px}.barlabel{font-size:10px;color:var(--muted);margin-top:5px}.feed{max-height:560px;overflow:auto}.event{padding:10px 0;border-bottom:1px solid var(--border)}.eventdetail{font-size:13px;color:var(--muted);margin-top:4px}table{width:100%;border-collapse:collapse;font-size:13px}th,td{text-align:left;padding:10px;border-bottom:1px solid var(--border)}th{color:var(--muted)}.scroll{overflow:auto}.hidden{display:none}.toolbar{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}@media(max-width:950px){.layout{grid-template-columns:1fr}.head{align-items:flex-start;flex-direction:column}.headright{width:100%}}
</style></head><body><div class="page"><div class="head"><div><h1>Tradie Invoice Admin</h1><div class="hint">Live business, user and Gemini monitoring</div></div><div class="headright"><a class="navbtn" href="/admin/railway">Railway Monitoring</a><a class="navbtn primary" href="/admin/controls">Admin Controls</a><div id="live" class="live">LIVE: Connecting...</div></div></div>
