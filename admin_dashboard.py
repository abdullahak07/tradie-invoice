...
async function refresh(){
 ...
 `${g.tokens_today} tokens -  ${g.failures_today} failures -  `+
 `${Math.round(g.average_latency_ms)}ms average`;
  invoice_value.textContent=money.format(s.invoice_value);
  user_hint.textContent=`${s.trial_users} trial - ${s.paid_users} paid`;
 ...
 <div class="event">
  ...
  document.getElementById('status').textContent='LIVE: Connected - '+new Date().toLocaleTimeString();
 }catch(e){
