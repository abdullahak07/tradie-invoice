...
async def telegram_webhook(request: Request) -> dict[str, bool]:
    ...
    if data.startswith("voice_") and chat_id:
        ...
        if not pending:
            await telegram_routes.send_telegram(chat_id, "That transcription expired. Send the voice note again.")
            return {"ok": True}
        transcript = str(pending["transcript"])
        action = data.split(":", 1)[0]
        if action == "voice_accept":
            voice_confirm.clear("telegram", chat_id)
            await telegram_routes.send_telegram(chat_id, "✅ Accepted. Creating your draft…")
        ...
        if action == "voice_edit":
            ...
            await telegram_routes.send_telegram(chat_id, "✏️ Send the corrected full transcription as text.")
            return {"ok": True}
        voice_confirm.clear("telegram", chat_id)
...
async def whatsapp_webhook(request: Request) -> dict[str, bool]:
    ...
    if action.startswith("voice_") and sender:
        ...
        if not pending:
            await whatsapp_routes.send_whatsapp_text(sender, "That transcription expired. Send the voice note again.")
            return {"ok": True}
        transcript = str(pending["transcript"])
        action_name = action.split(":", 1)[0]
        if action_name == "voice_accept":
            voice_confirm.clear("whatsapp", sender)
            await whatsapp_routes.send_whatsapp_text(sender, "✅ Accepted. Creating your draft…")
        ...
        if action_name == "voice_edit":
            ...
            await whatsapp_routes.send_whatsapp_text(sender, "✏️ Send the corrected full transcription as text.")
            return {"ok": True}
        voice_confirm.clear("whatsapp", sender)
        await whatsapp_routes.send_whatsapp_text(sender, "❌ Voice request cancelled.")
        return {"ok": True}
