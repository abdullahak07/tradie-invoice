...
def graphql_request(query: str, variables: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "tradie-invoice-admin/1.0",
        },
        method="POST",
    )
...
def deployment_history() -> dict[str, Any]:
    ...
    if not project_id or not service_id:
        return {
            "configured": False,
            "error": "RAILWAY_PROJECT_ID or RAILWAY_SERVICE_ID is missing",
            "deployments": [],
        }
    ...
    try:
        ...
        deployments = [edge["node"] for edge in data["deployments"]["edges"]]
        return {
            "configured": True,
            "error": "",
...
def service_resource_limits() -> dict[str, Any]:
    service_id = env_value("RAILWAY_SERVICE_ID")
    environment_id = env_value("RAILWAY_ENVIRONMENT_ID")
    if not service_id or not environment_id:
        return {"available": False, "limits": {}, "error": "Railway IDs missing"}

    candidate_queries = [
        (
            """
            query resourceLimits($serviceId: String!, $environmentId: String!) {
              serviceInstanceResourceLimits(serviceId: $serviceId, environmentId: $environmentId)
            }
            """,
    ...
    last_error = ""
    for query, variables in candidate_queries:
        try:
            data, _ = graphql_request(query, variables)
            return {
                "available": True,
                "limits": data.get("serviceInstanceResourceLimits", {}),
                "error": "",
            }
        except Exception as exc:
            last_error = str(exc)

    return {"available": False, "limits": {}, "error": last_error[:300]}


def public_health() -> dict[str, Any]:
    domain = env_value("RAILWAY_PUBLIC_DOMAIN")
    if not domain:
        return {"configured": False, "ok": False, "latency_ms": 0, "status": None}

    url = f"https://{domain}/whatsapp/health"
    started = time.perf_counter()
    try:
        request = urllib.request.Request(url, headers={"User-Agent": "tradie-invoice-monitor/1.0"})
...
def api_status(_: str = Depends(admin_login)) -> JSONResponse:
    ...
    payload = {
        ...
        "railway": {
            ...
            "token_configured": bool(env_value("RAILWAY_API_TOKEN", "RAILWAY_TOKEN")),
        },
        ...
        "resource_limits": service_resource_limits(),
    }
...
<body><div class="page">
<div class="head"><div><h1>Railway Monitoring</h1><div class="hint"><a href="/admin">Back to business dashboard</a></div></div><div id="live" class="live">LIVE: Connecting...</div></div>
<div class="grid">
...
<div class="panel"><h2>Recent deployments</h2><table><thead><tr><th>Status</th><th>Created</th><th>Deployment ID</th></tr></thead><tbody id="deployments"></tbody><div id="deployError" class="bad"></div></div>
</div>
<div class="panel"><h2>API and resource limits</h2><pre id="limits"></pre></div>
</div>
<script>
function text(id,v){document.getElementById(id).textContent=v}function timefmt(s){const d=new Date(s);return Number.isNaN(d.getTime())?s:d.toLocaleString('en-AU')}function duration(sec){let s=Math.max(0,Math.floor(sec));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);return `${d}d ${h}h ${m}m`}async function refresh(){try{const r=await fetch('/admin/railway/api/status',{cache:'no-store'});if(!r.ok)throw new Error(r.status);const d=await r.json();const rt=d.runtime;text('deployStatus',d.latest_deployment?.status||'Unknown');text('deployTime',d.latest_deployment?timefmt(d.latest_deployment.createdAt):'No deployment data');text('health',d.public_health.ok?'Healthy':'Unavailable');document.getElementById('health').className='value '+(d.public_health.ok?'ok':'bad');text('latency',`${d.public_health.latency_ms}ms - HTTP ${d.public_health.status??'n/a'}`);text('cpu',`${rt.container_cpu_percent.toFixed(2)}%`);text('cpuLimit',rt.cpu_limit_cores?`${rt.cpu_limit_cores} CPU cores limit`:'CPU limit not exposed');text('memory',`${rt.memory_percent.toFixed(1)}%`);text('memoryDetail',`${rt.formatted.memory_current} / ${rt.formatted.memory_limit}`);document.getElementById('memoryBar').style.width=Math.min(rt.memory_percent,100)+'%';text('disk',`${rt.disk_percent.toFixed(1)}%`);text('diskDetail',`${rt.formatted.disk_used} / ${rt.formatted.disk_total}`);document.getElementById('diskBar').style.width=Math.min(rt.disk_percent,100)+'%';text('uptime',duration(rt.uptime_seconds));text('deploymentId',d.railway.deployment_id||'Deployment ID unavailable');text('rx',rt.formatted.network_received);text('tx',rt.formatted.network_sent);text('rxRate',rt.formatted.network_received_per_second);text('txRate',rt.formatted.network_sent_per_second);text('config',JSON.stringify(d.railway,null,2));text('limits',JSON.stringify({resource_limits:d.resource_limits,api_rate_limit:d.rate_limit},null,2));document.getElementById('deployments').innerHTML=d.deployments.map(x=>`<tr><td>${x.status}</td><td>${timefmt(x.createdAt)}</td><td>${x.id}</td></tr>`).join('');text('deployError',d.deployment_error||'');text('live','LIVE: Connected - '+new Date().toLocaleTimeString())}catch(e){console.error(e);text('live','LIVE: Connection error')}}refresh();setInterval(refresh,5000);
</script></body></html>
"""
