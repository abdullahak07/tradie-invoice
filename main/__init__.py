...
def _safe_summary(data: dict, logo_path: str) -> str:
    ...
    logo_line = "Logo: Ready to use" if logo_path else (
        "Logo: Detected, but not clean enough to use"
        if bool(data.get("logo_visible"))
        else "Logo: Not found"
    )
...
_original_whatsapp = self_onboarding.whatsapp_webhook


async def _logo_enabled_whatsapp(request):
    ...
    text = str((message.get("text") or {}).get("body") or "").strip().upper()
    session = self_onboarding.get_session("whatsapp", sender)

    if session and text == "LOGO" and str(session["state"]) == "awaiting_confirmation":
        ...
        await whatsapp_routes.send_whatsapp_text(
            sender,
            "Send a clear standalone PNG or JPG of your business logo. Do not send the full invoice again.",
        )
        return {"ok": True}
    ...
    if session and str(session["state"]) == "awaiting_logo" and str(message.get("type") or "") in {"image", "document"}:
        ...
        folder = self_onboarding.business_onboarding.UPLOAD_DIR / (
...
self_onboarding.whatsapp_webhook = _logo_enabled_whatsapp
voice_confirm_routes.whatsapp_webhook = _logo_enabled_whatsapp
